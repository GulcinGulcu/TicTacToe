...
//In the game, when we put X, we are also going to add class x to that card to keep track of the cards. Because game works with variables, we just switch x and o, the rest is the same.
    ...
    function computerPlay() {
        ...
        if ((cardsContainsTwoO.length > 0 && twoX.length > 0) || cardsContainsTwoO.length > 0) {
            ...
            if (!elmWithoutClassO.classList.contains(classX)) { // if there is no X there, computer wins. not making defense.
                ...
                //commputer looks for danger and make defense here.
